...
<div class="form-group row">
    <div class="col-lg-12">
        ...
        <textarea class="form-control" id="finance_desc" name="finance_desc" placeholder="Acc Keuangan" required
            {{ $module_action == "Show" ? "readonly" : "" }}
            {{ $module_action == "Upload" ? "readonly" : "" }}>{{ $$module_name_singular->finance_desc ?? '' }}</textarea>
...
@if($status['status'] == 'Finish' || $status['status'] == 'Rejected Director' || $status['status'] == 'Transfer Process')
<div class="form-group row">
    <div class="col-lg-12">
        <label class="form-label font-weight-bold" for="director_desc">Deskripsi Direktur <span
                class="wajib">*</span></label>
        <textarea class="form-control" id="director_desc" name="director_desc" placeholder="Acc"
            readonly>{{ $$module_name_singular->director_desc ?? '' }}</textarea>
...
<div class="form-group row">
    <div class="col-lg-12">
        <label class="form-label" for="finance_attachment">Attachment</label>
        <input type="file" class="form-control" id="finance_attachment" name="finance_attachment" {{ $module_action == "Upload" ? "required" : "" }}>
        @if ($errors->has('finance_attachment'))
        <span class="text-danger">{{ $errors->first('finance_attachment') }}</span>
        @endif
    </div>
</div>
@endif
